...
foreach( $car->parts->partDrawData->variants as $pd ){
    $tabs[] = [
        'url' => "/illustration/?cat=$oid->catalog&mark={ $oid->mark }&model={ $oid->model }&production={ $oid->production }&group={ $oid->group }&subGroup={ $oid->subgroup }&tableCod=".urlencode(base64_encode($oid->table))."&variant=".$pd->variante,
